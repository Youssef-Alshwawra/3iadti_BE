...
export const register = async (req, res) => {
  try {
    ...
    if (role === "patient") {
      ...
      // Add clinic info if provided during registration
      if (additionalData.clinicName || additionalData.clinicAddress) {
        doctorData.clinic = {
          name: additionalData.clinicName,
          address: additionalData.clinicAddress,
          phone: additionalData.clinicPhone || phone,
          description: additionalData.clinicDescription,
          status: "pending",
        };
      }
    ...
    const response = {
      ...
      message: emailSent
        ...
        : "Registration successful. Email service temporarily unavailable - please contact support for manual verification.",
      userId: user._id, // Send user ID for OTP verification
      emailSent: emailSent,
    };

    // In development mode, include OTP for easy testing
    if (process.env.NODE_ENV === "development") {
      response.otp = otp;
      response.devMessage = "🧪 Development Mode: OTP included for testing";
    }
  ...
  try {
    ...
    const response = {
      ...
      message: emailSent
        ...
        : "OTP generated but email service temporarily unavailable - please contact support",
      emailSent: emailSent,
    };
...
export const forgotPassword = async (req, res) => {
  try {
    ...
    await sendEmail({
      to: email,
      subject: `Password Reset - Your OTP Code ${otp}`,
      html: `
                <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
